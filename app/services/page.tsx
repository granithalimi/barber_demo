...
export default async function Page() {
  ...
  //   }
  // }
  const services = await supabase.from('services').select("*").order("id", { ascending: true })
  return (
    <main className="bg-gradient-to-tl from-gray-900 to-gray-800 min-h-screen text-white">
      ...
      <h1 className={`${montserrat.className} text-center text-2xl mb-5`}>Services</h1>
      <form method="POST" className="w-full flex flex-col items-center gap-3">
      ...
      <div className="mx-auto mt-10 w-11/12 md:w-2/3 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
        {services.data && services.data.length > 0 && (
          <>
            {services.data.map((s, ind) => (
              <div className="p-3 border-2 border-white rounded-lg flex flex-col items-center justify-between" key={ind}>
                <div className="mb-3">
                  <h1 className={`${montserrat.className} text-center text-xl`}>{s.name}</h1>
                </div>
                <div className="mb-3">
                  <h1 className="text-center text-green-500">{s.price} Den</h1>
                  <h1 className="text-center text-red-500">{s.time * 30} min</h1>
                </div>
